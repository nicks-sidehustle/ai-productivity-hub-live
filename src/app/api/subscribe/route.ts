...
const BREVO_LIST_ID = parseInt(process.env.BREVO_LIST_ID || '16')
const WELCOME_TEMPLATE_ID = 7
export async function POST(request: NextRequest) {
  try {
    ...
    const contactResponse = await fetch('https://api.brevo.com/v3/contacts', {
      ...
      body: JSON.stringify({ email, listIds: [BREVO_LIST_ID], attributes: { SOURCE: source || 'oneclickai', SIGNUP_DATE: new Date().toISOString() }, updateEnabled: true }),
    })
